# Sampling-based Algorithms RRT, RRT* and Informed RRT*
...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x_center = Node((self.start.row+self.goal.row)/2.0, (self.start.col+self.goal.col)/2.0)
            ...
            x_ball = [radiusX * np.random.random() * np.cos(2 * np.pi * np.random.random()), radiusY * np.random.random() * np.sin(2 * np.pi * np.random.random())]

            # Map ball sample to the ellipsoid - x_rand
            x_rand = np.dot(C, x_ball)

            point = [x_rand[0] + x_center.row, x_rand[1] + x_center.col]
    ...
    def path_cost(self, start_node, end_node):
        ...
        while start_node.row != curr_node.row or start_node.col != curr_node.col:
            # Keep tracing back until finding the start_node or no path exists
...
if __name__ == "__main__":
    ...
    # Search with RRT, RRT* and Informed RRT*
